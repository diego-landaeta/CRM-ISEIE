...
## 2026-06-03 — Tanda backlog: #17pt2 + #12 + #13 + #18 (todas aprobadas en QA)

Aprobado por Diego tras probar las 4 features. Desplegado en ISEIE + ISEIH.

- `feat(audit):` **#17 pt2** — tabla `lead_audit_log` (072 ISEIE / 070 ISEIH). `lead.service.updateLead` captura snapshot pre-update y inserta una fila por campo cambiado (nombre/email/telefono/notas/producto/canal). `findById` devuelve `lead.auditLog`. `useLeads` mergea con statusHistory en timeline ordenado por timestamp, render con etiquetas humanas + emojis (`📝 Email: x → y · por Adriana`).
- `feat(merge):` **#12** — endpoint y dialog ya existían (`POST /leads/:id/merge` + `MergeLeadDialog`); estaban sin conectar a la UI. Añadido botón "Fusionar duplicado" en `LeadSidebar` (admin/superadmin, icono GitMerge). Ampliado el merge: ahora también mueve `lead_audit_log` del loser al winner, deja nota en AMBOS leads (winner: `🔗 Fusionado con #X`, loser: `❌ Fusionado en #Y` — visible si se restaura desde papelera), registra audit `fusion_winner`/`fusion_loser`, soft-delete con `deleted_reason='duplicado_manual'` + motivo + by, y notifica admin (`notifyAdmins type='lead_merged'`).
- `feat(dup-review):` **#13** — migración 073/071 `lead_duplicate_review_queue` (UNIQUE lead_id, estados pending|approved|merged|rejected). Nuevo `dup-queue.service.js` con enqueue/list/counts/decide/markMerged. Webhook (lead.service createFromWebhook): tras crear lead duplicado, `dupQueue.enqueue()` fire-and-forget — Make sigue recibiendo 200 OK <500ms. `mergeLeads` llama `markMerged(loserId)`. Endpoints: GET `/leads/review-queue` + POST `/leads/review-queue/:id/decide`. Frontend `DupReviewQueuePage` con tabs Pendientes/Aprobados/Fusionados/Descartados + cards side-by-side (nuevo vs original) + 3 acciones (Aprobar / Fusionar al original / Descartar). Sidebar admin nueva entrada "Revisión duplicados". `MergeLeadDialog` acepta nueva prop `initialLoserId` para pre-seleccionar el loser desde la cola.
- `feat(leads):` **#18** — migración 074/072 `lead_products` (N:N con `responsable_id` opcional, `status`, `notas`, `added_via` enum manual/auto_reincidente/webhook/wc). `leads.producto_interes_id` sigue siendo el principal (backward-compat). Nuevo `lead-products.service.js` con listForLead/addProduct/updateProduct/removeProduct/autoAddFromReincidente. Endpoints CRUD `/leads/:id/products[/:lpId]`. `findById` devuelve `lead.secondaryProducts`. Hook automático en webhook: cuando llega un duplicado por email/tel con OTRO producto distinto al original, se añade al original como secundario (label auto_reincidente) — admin lo ve unificado en una ficha. Nuevo `LeadProductsCard` en la ficha con principal (estrella) + lista secundarios + form inline add + edit/remove.
